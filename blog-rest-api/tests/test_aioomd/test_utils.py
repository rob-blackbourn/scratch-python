...
def test_import_class():
    ...
    Document = utils.import_class('aioodm.document.Document')
    assert Document is aioodm.Document
    assert 'aioodm.document.Document' in utils.CLASSES_CACHE

    OtherDocument = utils.import_class('aioodm.document.Document')
...
def test_import_class_invalid_class_name():
    with pytest.raises(ImportError) as excinfo:
        utils.import_class('aioodm.WrongNameClass')

    expected = "No class named 'aioodm.WrongNameClass'"
    assert str(excinfo.value) == expected

    assert 'aioodm.WrongNameClass' not in utils.CLASSES_CACHE
